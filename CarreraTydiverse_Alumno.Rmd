...
cran %>% filter(package == "PT" | package == "ES")
...
cran3 %>% mutate(size_mb = size / 2^20) %>% mutate(size_gb = size_mb / 2^10)
...
top_unique_sorted <- top_counts_sorted %>% arrange(unique)
top_unique_sorted
...
cran %>% mutate(size_mb = size / 2^20) %>% filter(size <= 0.5) %>% arrange(desc(size_mb))
...
cran %>% group_by(country) %>% summarise(total_size = sum(size)) %>% filter(total_size > quantile(total_size, probs = 0.9)) %>% ggplot(aes(x = country, y = total_size, colour = country)) + geom_point()
...
cran_filt <- cran %>% filter(!is.na(country))
code_paises <- code_paises %>% filter(!is.na(country_iso_code))
cran_join <- cran_filt %>% left_join(code_paises %>% select(country_iso_code, continent_name), by = c("country" = "country_iso_code"))
cran_join %>% group_by(continent_name) %>% summarise(total_size_tb = sum(size) / 2^40) %>% arrange(desc(total_size_tb)) %>% ggplot(aes(x = continent_name, y = total_size_tb, colour = continent_name)) + geom_bar(stat = "identity")
...
students2 %>% gather(key = "class", value = "count", -grade) %>% separate(class, into = c("sex", "class"))
...
students3
students3 %>% gather(class, score, -name, na.rm = TRUE) %>% separate(class, into = c("class", "type"))
...
parse_number('class5')
#students3 %>%   mutate(class = parse_number(class))
...
student_info <- students4 %>% select(id, name, sex) %>% distinct()
...
bind_rows(passed %>% mutate(status = "passed"), failed %>% mutate(status = "failed"))
...
sat %>% gather(key = "part_sex", value = "count", -score_range) %>% separate(part_sex, into = c("part", "sex"))
...
sat <- sat %>% gather(key = "part_sex", value = "count", -score_range) %>% separate(part_sex, into = c("part", "sex")) %>% group_by(part, sex) %>% mutate(total = sum(count), prop = count / total)
...
sat %>% ggplot(aes(x = score_range, y = count, color = sex)) + geom_point() + facet_wrap(~part, ncol=1) 
...
singapore <- ymd("2008-06-17", tz = "Asia/Singapore")
interval(singapore, llegada_hk) %>% as.period()
